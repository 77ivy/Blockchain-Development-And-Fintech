...
def gen_merkle_proof(leaves, pos):
    ...
    for level in range(height):
        ...
        ## length is the number of elements on the last level.
        length = len(state)- pre


        i = 0
        ## update the newest level
        for i in range(int(length/2)):
            a=pre
            state.append(hash_internal_node(state[2*i+a], state[2*i+a+1]))

        ## pre is total number of element of all previous level(not including the newest level)
        pre = length+pre

        ## we should check that the node is right or left
        if level != 0:
            if level_pos % 2:
